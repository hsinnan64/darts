...
同一鏢的四條線共用一個顏色，下一鏢換色（`THROW_COLORS`，6 色循環：
橘、黃、綠、青、藍、紫）。

**紅、白、黑是使用者明確排除的**，不要加回去：紅容易和畫面上的警示、
鏢靶、衣物混淆；白跟骨架頸線同色；黑在純黑背景模式下看不見。

調色盤是算出來的不是憑感覺挑的：色相盡量拉開（最小 30 度），亮度逐色
微調讓純黑背景上的對比都在 6.5:1 以上。**要改顏色請一併驗算色相間距、
黑底對比與飽和度**，純藍在黑底上特別容易暗到看不清楚，而飽和度不足的
淺色會看起來接近白色。
